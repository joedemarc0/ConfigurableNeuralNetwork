...
Matrix Network::Layer::backward(const Matrix& dA, size_t batch_size, double learning_rate) {
    ...
    if (isLastLayer) {
        if (actType != Activations::ActivationType::SOFTMAX) {
            throw std::runtime_error("Output Layer must utilize Softmax actType");
        }
...
Matrix Network::forward(const Matrix& X) {
    if (!isCompiled) {
        ...
        layers.emplace_back(last_input_size, 10, Activations::ActivationType::SOFTMAX, InitType::XAVIER, true);
        isCompiled = true;
    }
...
// Private addLayer for adding output layer
...
void Network::addLayer(size_t neurons) {
    ...
    if (networkActType == Activations::ActivationType::SOFTMAX) {
        throw std::runtime_error("Network Activation Function Type shouldn't be Softmax");
    } else if (isCompiled) {
        throw std::runtime_error("Cannot add layers after Network is compiled");
    }
